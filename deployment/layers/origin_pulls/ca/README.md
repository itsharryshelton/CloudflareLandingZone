...
so the `origin_trust_bundle` output is complete without a network fetch at plan
time. It is a certificate, not a key: it is public material, published by
...
| Subject | `CN=origin-pull.cloudflare.net, OU=Origin Pull, O=CloudFlare, Inc., C=US` |
| Expires | 2029-11-01 |
