...
- **Resource Tagging:** Zones, Access applications, R2 buckets, KV namespaces and Workers are tagged from `accounts/<account>/tags.tfvars`, validated at plan time, and written after apply by a dedicated job ([`resource-tags.sh`](.github/scripts/resource-tags.sh)) until the provider ships a tagging resource. Every managed resource carries `managed-by` and `layer`, so anything created by hand stands out. See [Resource tags](.github/workflows/README.md#resource-tags).
